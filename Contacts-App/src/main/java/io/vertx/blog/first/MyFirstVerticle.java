...
public class MyFirstVerticle extends AbstractVerticle {
  ...
  private void startWebApp(Handler<AsyncResult<HttpServer>> next) {
    ...
    router.post("/api/contacts").handler(this::addOne);
    router.get("/api/contacts/:id").handler(this::getOne);
    router.put("/api/contacts/:id").handler(this::updateOne);
    router.delete("/api/contacts/:id").handler(this::deleteOne);
  ...
  private void addOne(RoutingContext routingContext) {
    final Contacts contacts = Json.decodeValue(routingContext.getBodyAsString(),
        Contacts.class);

    mongo.insert(COLLECTION, contacts.toJson(), r ->
        routingContext.response()
            .setStatusCode(201)
            .putHeader("content-type", "application/json; charset=utf-8")
            .end(Json.encodePrettily(contacts.setId(r.result()))));
  }
    ...
    if (id == null) {
      ...
      mongo.findOne(COLLECTION, new JsonObject().put("_id", id), null, ar -> {
        if (ar.succeeded()) {
          ...
          Contacts contacts = new Contacts(ar.result());
          routingContext.response()
              .setStatusCode(200)
              .putHeader("content-type", "application/json; charset=utf-8")
      ...
      mongo.update(COLLECTION,
          ...
          v -> {
            if (v.failed()) {
              ...
              routingContext.response()
                  .putHeader("content-type", "application/json; charset=utf-8")
                  .end(Json.encodePrettily(new Contacts(id, json.getString("name"), json.getString("number"))));
            }
          });
    }
  }
  ...
  private void getAll(RoutingContext routingContext) {
    mongo.find(COLLECTION, new JsonObject(), results -> {
      ...
      List<Contacts> contacts = objects.stream().map(Contacts::new).collect(Collectors.toList());
      routingContext.response()
          .putHeader("content-type", "application/json; charset=utf-8")
          .end(Json.encodePrettily(contacts));
    });
  ...
  private void createSomeData(Handler<AsyncResult<Void>> next, Future<Void> fut) {
    Contacts bowmore = new Contacts("Li Si", "188 8888 8888");
    Contacts talisker = new Contacts("Zhang San", "177 7777 7777");

...
public class MyFirstVerticleTest {
  ...
  public void checkThatTheIndexPageIsServed(TestContext context) {
    ...
    vertx.createHttpClient().getNow(port, "localhost", "/assets/index.html", response -> {
      ...
      response.bodyHandler(body -> {
        context.assertTrue(body.toString().contains("<title>Contacts</title>"));
        async.complete();
      });
    });
  }
  ...
  public void checkThatWeCanAdd(TestContext context) {
    Async async = context.async();
    final String json = Json.encodePrettily(new Contacts("Zhang San", "177 7777 7777"));
    vertx.createHttpClient().post(port, "localhost", "/api/contacts")
        ...
        .handler(response -> {
          ...
          response.bodyHandler(body -> {
            final Contacts whisky = Json.decodeValue(body.toString(), Contacts.class);
            context.assertEquals(whisky.getName(), "Zhang San");
            context.assertEquals(whisky.getNumber(), "177 7777 7777");
